  ...
  const getAllJeeps = async () => {
    try {
      ...
      let cars = await axios({
        method: "GET",
        url: `${baseURL}/vehicle/get/jeeps`,
      });
      console.log(cars);
      setJeeps(cars.data.data);
      setLoading(false);
    } catch (error) {
      ...
      <main>
        <section>
          <h2 className="font-Comfortaa text-3xl font-semibold border-b px-[120px] py-[30px] border-gray-400">
            Which jeep do you want to ride?
          </h2>
          <div className="flex items-center justify-between gap-5 flex-wrap flex-row-4 px-[100px]">
            {loading ? (
